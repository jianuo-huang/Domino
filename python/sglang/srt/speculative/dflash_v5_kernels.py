"""Triton kernels for the DFLASH causal_v5 sequential rollout.

The per-step body of v5 rollout is dominated by tiny ops at bs=1. Even with a
...
launches. These kernels collapse the small ops:
  ...
  1. ``v5_fused_silu_fc2_argmax`` — fuses ``SiLU(z+s) @ fc2.T + fc2_bias +
  ...
  2. ``v5_fused_gru_cell`` — fuses sigmoid/tanh/gate of the GRU cell into one
     kernel.
